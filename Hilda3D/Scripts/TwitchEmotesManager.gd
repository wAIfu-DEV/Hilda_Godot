...
func _process(_delta: float) -> void:
    ...
    for worker in workers:
        if worker.is_done:
            worker.done.emit()
            for handler: CacheAwaitHandler in awaiting_caching[worker.emote_name]:
                handler.done.emit()
...
func asyncFetchEmote(emote_name: String)-> Texture2D:
    ...
    if is_gif:
        ...
        var img = Image.new()
        # Gotta do that since Godot will print an error if cannot parse .webp data
        # I don't care if it does or doesn't since we handle the error
        #var err = img.load_webp_from_buffer(request[3])
        var err = img.load_png_from_buffer(request[3])
        if err != OK:
            pass
            #var png_data = await _asyncFetchPngFallback(id)
